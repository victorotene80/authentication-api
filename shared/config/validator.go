...
func (c *Config) validateTracing() error {
	if c.Tracing.Enabled {
		if c.Tracing.Endpoint == "" {
			return fmt.Errorf("OTLP_ENDPOINT is required when tracing is enabled")
		}
...
func (c *Config) validateLogging() error {
	if c.Logging.LogstashEnabled {
		if c.Logging.LogstashHost == "" {
			return fmt.Errorf("LOGSTASH_HOST is required when Logstash is enabled")
		}
		if c.Logging.LogstashPort <= 0 || c.Logging.LogstashPort > 65535 {
			return fmt.Errorf("LOGSTASH_PORT must be between 1 and 65535")
		}
		if c.Logging.LogstashProtocol != "tcp" && c.Logging.LogstashProtocol != "udp" {
			return fmt.Errorf("LOGSTASH_PROTOCOL must be either 'tcp' or 'udp'")
		}
	}
	return nil
}
